...
table_user_group_metrics = Table(
    'user_group_metrics', meta_data,
    create_pk('user_group_metric_id'),
    create_str('user_group_id', 50, True), create_str('metric_id', 50, True),
    create_tenant_id(), *create_tuple_audit_columns(), create_optimistic_lock()
)
table_spaces = Table(
    'spaces', meta_data,
...
table_topic_tags = Table(
    'topic_tags', meta_data,
    create_pk('topic_tag_id'),
    create_tuple_id_column('topic_id'),
    create_str('tag_name', 50, False),
...
table_collector_batch_data_shard = Table(
    'collector_batch_data_shard', meta_data,
    create_pk('shard_id'),
    create_str('name', 100, False),
    create_str('table_name', 100, False),
    create_int('status'),
    create_json('result'),
    create_int('type'),
    create_tuple_id_column('start_id', False), create_tuple_id_column('end_id', False),
    create_tenant_id(), *create_tuple_audit_columns(), create_optimistic_lock()
)
...
table_action_types = Table(
    'action_types', meta_data,
    create_pk('action_type_id'),
    create_str('name', 128),
    create_str('code', 50),
    create_description(),
    create_bool('requires_approval'),
    create_bool('enabled'),
    create_str('category', 50),
    create_json('parameters'),
    create_tenant_id(), *create_tuple_audit_columns(), create_optimistic_lock()
)

table_suggested_actions = Table(
    'suggested_actions', meta_data,
    create_pk('suggested_action_id'),
    create_str('name', 128),
    create_tuple_id_column('type_id'),
    create_str('risk_level', 20),
    create_description(),
    create_str('expected_outcome', 1024),
    create_json('conditions'),
    create_str('execution_mode', 20),
    create_str('priority', 20),
    create_bool('enabled'),
    create_int('execution_count'),
    create_number('success_rate'),
    create_datetime('last_executed'),
    create_json('parameters'),
    create_tenant_id(), *create_tuple_audit_columns(), create_optimistic_lock()
)
