...
def find_shard_key_factor(topic: Topic) -> Optional[Factor]:
	if topic.factors is None:
		return None
	shard_factors = ArrayHelper(topic.factors) \
		.filter(lambda x: x.keyType == FactorKeyType.PARTITION) \
		.sort_by(lambda x: (x.keyIndex if x.keyIndex is not None else 0)) \
		.to_list()
	if len(shard_factors) == 0:
		return None
	if len(shard_factors) > 1:
		raise ValueError(
			f'Only one factor can be marked as keyType=PARTITION in topic[name={topic.name}]; '
			f'found {len(shard_factors)}.'
		)
	return shard_factors[0]


def build_shard_key_clause(topic: Topic) -> str:
	shard_factor = find_shard_key_factor(topic)
	if shard_factor is None:
		return ''
	return f'\nSHARD KEY ({ask_column_name(shard_factor)})'
...
def build_table_script(topic: Topic) -> str:
	entity_name = as_table_name(topic)
	shard_key = find_shard_key_factor(topic)
	if shard_key is not None and is_raw_topic(topic):
		raise ValueError(
			f'Raw topic[name={topic.name}] cannot have a shard key. '
			f'Raw topics are stored as JSON and do not participate in sharding.'
		)
	shard_key_clause = build_shard_key_clause(topic)
	return f'''
