...
class AuroraLimitlessDataSourceParams(DataModel):
	echo: bool = False
	poolRecycle: int = 3600


class AuroraLimitlessDataSourceHelper(DataSourceHelper):
	def __init__(
	...
	@staticmethod
	def acquire_engine_by_url(url: str, params: AuroraLimitlessDataSourceParams) -> Engine:
	...
	def acquire_engine_by_params(
			...
			params: AuroraLimitlessDataSourceParams
	) -> Engine:
		url_params = AuroraLimitlessDataSourceHelper.build_url_params(data_source_params)
		search = AuroraLimitlessDataSourceHelper.build_url_search(url_params)
		if is_not_blank(search):
			search = f'?{search}'
		encoded_password = urllib.parse.quote_plus(password)
		url = f'postgresql+psycopg2://{username}:{encoded_password}@{host}:{port}/{name}{search}'
		return AuroraLimitlessDataSourceHelper.acquire_engine_by_url(url, params)
	...
	def build_url_params(data_source_params: Optional[List[DataSourceParam]]) -> Optional[List[DataSourceParam]]:
		...
		charset = AuroraLimitlessDataSourceHelper.find_param(url_params, 'client_encoding')
		if is_blank(charset):
			url_params = AuroraLimitlessDataSourceHelper.append_param(url_params, 'client_encoding', 'utf8')

		return url_params

	def acquire_storage(self) -> StorageAuroraLimitless:
		return StorageAuroraLimitless(self.engine)

	def acquire_topic_data_storage(self) -> TopicDataStorageAuroraLimitless:
		return TopicDataStorageAuroraLimitless(self.engine)
