...
class QueryAnalysisDataPage(DataPage):
	data: List[ObjectiveAnalysis]


@router.post('/indicator/objective-analysis/name', tags=[UserRole.ADMIN], response_model=QueryAnalysisDataPage)
async def find_analysis_page_by_name(
		query_name: Optional[str], pageable: Pageable = Body(...),
		principal_service: PrincipalService = Depends(get_admin_principal)) -> QueryAnalysisDataPage:
	analysis_service = get_analysis_service(principal_service)

	def action() -> QueryAnalysisDataPage:
		tenant_id: TenantId = principal_service.get_tenant_id()
		if is_blank(query_name):
			# noinspection PyTypeChecker
			return analysis_service.find_page_by_text(None, tenant_id, pageable)
		else:
			# noinspection PyTypeChecker
			return analysis_service.find_page_by_text(query_name, tenant_id, pageable)
