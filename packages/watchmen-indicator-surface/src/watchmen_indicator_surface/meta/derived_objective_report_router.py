...
from watchmen_indicator_kernel.meta.derived_objective_report_service import DerivedObjectiveReportService
...
@router.get(
	'/indicator/derived-objective/report/connect', tags=[UserRole.CONSOLE, UserRole.ADMIN], response_model=DerivedObjectiveReport)
async def connect_as_derived_objective_report(
	...
	def action() -> DerivedObjectiveReport:
		...
		derived_objective_report = DerivedObjectiveReport(name=name, objectiveReportId=objective_report_id, definition=objective_report)
		...
		derived_objective_report: DerivedObjectiveReport = derived_objective_report_service.create(derived_objective_report)
		return derived_objective_report

	return trans(derived_objective_report_service, action)


@router.post('/indicator/derived-objective-report', tags=[UserRole.CONSOLE, UserRole.ADMIN], response_model=DerivedObjectiveReport)
async def save_derived_objective_report(
		derived_objective_report: DerivedObjectiveReport, principal_service: PrincipalService = Depends(get_console_principal)
) -> DerivedObjectiveReport:
	derived_objective_report_service = get_derived_objective_report_service(principal_service)
	...
	def action(a_derived_objective_report: DerivedObjectiveReport) -> DerivedObjectiveReport:
		...
		if derived_objective_report_service.is_storable_id_faked(a_derived_objective_report.derivedObjectiveId):
			...
			# noinspection PyTypeChecker
			a_derived_objective_report: DerivedObjectiveReport = derived_objective_report_service.create(a_derived_objective_report)
		else:
			# noinspection PyTypeChecker
			...
			# noinspection PyTypeChecker
			a_derived_objective_report: DerivedObjective = derived_objective_report_service.update(a_derived_objective_report)
...
@router.get(
	'/indicator/derived-objective-report/list', tags=[UserRole.CONSOLE, UserRole.ADMIN], response_model=List[DerivedObjectiveReport])
...
@router.get('/indicator/derived-objective-report/rename', tags=[UserRole.CONSOLE, UserRole.ADMIN], response_class=Response)
...
@router.get('/indicator/derived-objective-report/delete', tags=[UserRole.CONSOLE, UserRole.ADMIN], response_class=Response)
