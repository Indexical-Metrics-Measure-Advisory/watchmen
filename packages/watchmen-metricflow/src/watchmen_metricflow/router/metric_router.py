...
from watchmen_metricflow.service.meta_service import load_metrics_by_tenant_id_sync, \
    load_semantic_models_by_tenant_id_sync, build_profile, get_metric_service
...
# MetricFlowResponse is imported from the model layer and re-exported here for
# backward compatibility (existing imports from this module keep working)
...
async def list_metrics(principal_service: PrincipalService = Depends(get_admin_principal))->MetricListResponse:
    ...
    response = await run_in_threadpool(find_all_metrics, config)
    ...
    meta_metrics = await run_in_threadpool(load_metrics_by_tenant_id_sync, principal_service)
...
async def find_dimensions_by_metric(metric_name: str,principal_service: PrincipalService = Depends(get_console_principal))->DimensionListResponse:
    ...
    return await run_in_threadpool(load_dimensions_by_metrics, [metric_name], config)
...
async def find_dimensions(metrics: List[str],principal_service: PrincipalService = Depends(get_admin_principal))->DimensionListResponse:
    ...
    return await run_in_threadpool(load_dimensions_by_metrics, metrics, config)
...
async def get_metric_value(req :MetricQueryRequest,
    ...
    # cfg = data_config_loader.get_config()
    # explicit request order wins; default is metric value descending
    order = req.order if req.order else ["-" + req.metric]
    query_result: MetricFlowQueryResult = await run_in_threadpool(
        query,
        cfg=config,
        metrics=[req.metric],
        group_by=req.group_by,
...
async def query_metrics(request_list: List[MetricQueryRequest],
    ...
    # built lazily: skipped entirely when every request is served by the direct bypass
    config: Optional[CLIConfigurationDB] = None
...
async def build_metric_config(principal_service):
    """Build (or fetch from cache) the tenant dbt CLI configuration.

    The body is synchronous meta storage work; run it in the threadpool so the
    event loop is never blocked by meta queries.
    """
    return await run_in_threadpool(_build_metric_config_sync, principal_service)


def _build_metric_config_sync(principal_service):
    ...
    if cached is not None:
        return cached
    metrics: List[Metric] = load_metrics_by_tenant_id_sync(principal_service)
    ## convert to json
    metrics_json = [item.model_dump() if hasattr(item, 'model_dump') else item for item in metrics]
    semantics: List[SemanticModel] = load_semantic_models_by_tenant_id_sync(principal_service)
