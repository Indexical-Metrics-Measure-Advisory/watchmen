...
from watchmen_collector_batch.common import ask_batch_collector_monitor_event_wait
...
class EventListener:
    ...
    def create_thread(self, scheduler: BackgroundScheduler=None) -> None:
        wait_sec = ask_batch_collector_monitor_event_wait()
        trigger = IntervalTrigger(seconds=wait_sec)
        scheduler.add_job(
            EventListener.event_loop_run,
            trigger=trigger,
            args=(self,),
            max_instances=1,
            coalesce=True,
