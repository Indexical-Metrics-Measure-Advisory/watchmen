...
from watchmen_collector_batch.model import BatchTriggerEvent, QueryParam
...
class BatchBatchTriggerEventShaper(EntityShaper):
	...
	def serialize_params(params: Optional[List[QueryParam]]) -> Optional[list]:
		if params is None:
			return None
		return ArrayHelper(params).map(lambda x: BatchBatchTriggerEventShaper.serialize_param(x)).to_list()

	def serialize(self, entity: BatchTriggerEvent) -> EntityRow:
		return TupleShaper.serialize_tenant_based(entity, {
			...
			'params': BatchBatchTriggerEventShaper.serialize_params(entity.params),
			'result': entity.result
		})

	def deserialize(self, row: EntityRow) -> BatchTriggerEvent:
		# noinspection PyTypeChecker
		return TupleShaper.deserialize_tenant_based(row, BatchTriggerEvent(
...
BATCH_TRIGGER_EVENT_TABLE = 'batch_trigger_event'
BATCH_TRIGGER_EVENT_ENTITY_SHAPER = BatchBatchTriggerEventShaper()


class BatchBatchTriggerEventService(TupleService):

	def should_record_operation(self) -> bool:
		return False

	def get_entity_name(self) -> EntityName:
		return BATCH_TRIGGER_EVENT_TABLE

	def get_entity_shaper(self) -> EntityShaper:
		return BATCH_TRIGGER_EVENT_ENTITY_SHAPER

	def get_storable_id_column_name(self) -> EntityName:
		return 'event_trigger_id'

	def get_storable_id(self, storable: BatchTriggerEvent) -> StorableId:
		# noinspection PyTypeChecker
		return storable.eventTriggerId

	def set_storable_id(
			self, storable: BatchTriggerEvent, storable_id: EventTriggerId) -> Storable:
		storable.eventTriggerId = storable_id
		return storable

	def create_trigger_event(self, trigger_event: BatchTriggerEvent) -> BatchTriggerEvent:
	...
	def update_trigger_event(self, trigger_event: BatchTriggerEvent) -> Optional[BatchTriggerEvent]:
		self.begin_transaction()
		try:
	...
	def find_event_by_id(self, event_trigger_id: EventTriggerId) -> BatchTriggerEvent:
		try:
	...
	def find_executing_events(self) -> List[BatchTriggerEvent]:
		try:
			self.begin_transaction()
			# noinspection PyTypeChecker
	...
	def find_initial_event_by_tenant_id(self, tenant_id: str) -> Optional[BatchTriggerEvent]:
		self.begin_transaction()
		try:
	...
	def find_executing_event_by_tenant_id(self, tenant_id: str) -> Optional[BatchTriggerEvent]:
		self.begin_transaction()
		try:
			res = self.storage.find(self.get_entity_finder(
				criteria=[
	...
	def find_last_finished_schedule_event_by_tenant_id(self, tenant_id: str) -> Optional[BatchTriggerEvent]:
		try:
			self.storage.connect()
...
def get_batch_trigger_event_service(storage: TransactionalStorageSPI,
                              snowflake_generator: SnowflakeGenerator,
                              principal_service: PrincipalService
                              ) -> BatchBatchTriggerEventService:
	return BatchBatchTriggerEventService(storage, snowflake_generator, principal_service)
