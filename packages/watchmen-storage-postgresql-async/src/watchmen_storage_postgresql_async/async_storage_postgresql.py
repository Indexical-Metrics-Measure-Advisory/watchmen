...
from watchmen_storage_postgresql.table_creator import build_columns_script, build_indexes_script, \
...
class AsyncStoragePostgreSQL(AsyncStorageRDS):
...
class AsyncTopicDataStoragePostgreSQL(AsyncStoragePostgreSQL, AsyncTopicDataStorageRDS):
	...
	async def ask_reflect_factors(self, table_name: str, schema: str) -> List[Factor]:
		try:
			await self.connect()
			columns = (await self.connection.execute(text(self.ask_reflect_sql(table_name, schema)))).mappings().all()
