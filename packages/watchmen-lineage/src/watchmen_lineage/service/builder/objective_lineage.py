...
from watchmen_model.indicator import Objective, ObjectiveFactor, ObjectiveFactorOnIndicator, \
	ObjectiveFactorOnComputation, ComputedObjectiveParameter, ObjectiveParameter, ReferObjectiveParameter, \
...
class ObjectiveLineageBuilder(LineageBuilder):
	...
	def __process_objective_factors(self,graphic,objective:Objective,objective_facet:ObjectiveFacet,principal_service:PrincipalService):
		for objective_factor in objective.factors:
			objective_factor_facet: ObjectiveFactorFacet = ObjectiveFactorFacet(nodeId=objective_factor.uuid,parentId=objective.objectiveId)
	...
	def __process_objective_factor(self,graphic,objective_factor:ObjectiveFactor,objective_facet:ObjectiveFacet,principal_service:PrincipalService):
		if objective_factor.kind == ObjectiveFactorKind.INDICATOR:
			...
			graphic_builder.add_edge_with_source_and_target(graphic,indicator_facet,objective_factor_facet,RelationType.Direct,LineageType.OBJECTIVE)
			...
			self.__process_formula(graphic,objective_factor.formula,objective_factor_facet,objective_facet,principal_service)


	def __process_formula(self,graphic,computation_factor:ComputedObjectiveParameter,objective_factor_facet:ObjectiveFactorFacet,objective_facet,principal_service):
		for objective_parameter in computation_factor.parameters:
			self.__process_objective_parameter(graphic,objective_parameter,objective_factor_facet,objective_facet,principal_service)

	def __process_objective_parameter(self,graphic,objective_parameter:ObjectiveParameter,objective_factor_facet:ObjectiveFactorFacet,objective_facet:ObjectiveFacet,principal_service):
		if isinstance(objective_parameter,ReferObjectiveParameter):
			...
			if objective_parameter.uuid in objective_facet.objectiveFactorHolders:
				...
				self.__process_objective_factor(graphic,objective_factor,objective_factor_facet,principal_service)
			...
			for parameter in objective_parameter.parameters:
				self.__process_objective_parameter(graphic,parameter,objective_factor_facet,objective_facet,principal_service)
