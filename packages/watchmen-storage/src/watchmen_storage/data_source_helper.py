...
SECRET_MANGER_KEY = [HOST, PORT, USERNAME, PASSWORD, NAME, SECRET_TYPE, AWS_REGION_NAME, AWS_ACCESS_KEY_ID,
                     AWS_ACCESS_SECRET_ID, SECRET_ID]
...
class DataSourceHelper:
    ...
    def find_param(params: Optional[List[DataSourceParam]], key: str) -> Optional[str]:
        ...
        for param in params:
            if is_not_blank(param.name) and param.name.strip().lower() == key.strip().lower():
                value = param.value
...
def build_secrets_manager(data_source_params: Optional[List[DataSourceParam]]) -> SecretsManger:
    secret_params = ArrayHelper(data_source_params).filter(lambda x: x.name in SECRET_MANGER_KEY).to_map(
        lambda x: x.name,
        lambda x: x.value)
