...
class SubjectDataLoader(EventDataLoader):
	...
	def load_data_from_event_sources(self, event_source: EventSource, source_id: str,tenant_principal_service: PrincipalService):
		if not self.__is_subject_event_source(self, event_source):
			...
			subject_data = SubjectDataService(subject, tenant_principal_service).page(Pageable(pageNumber=1,pageSize=100))
