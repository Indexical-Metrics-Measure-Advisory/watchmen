            ...
            batch_entries = [
                {
                    ...
                    **{k: v for k, v in msg.items() if k == 'MessageAttributes'}
                }
                ...
                try:
                    ...
                    if 'Failed' in response:
                        ...
                        if messages_to_retry and attempt < self.max_retries:
                            delay = self._exponential_backoff_delay(attempt)
                            self.logger.warning(
                                f"partial messages will retry after {delay:.2f} secs ({attempt + 1}/{self.max_retries + 1})"
                            )
                            time.sleep(delay)
                            ...
                            batch_entries = [
                                ...
                                for idx, msg in enumerate(messages_to_retry)
                            ]
                        else:
                            failures.extend([
