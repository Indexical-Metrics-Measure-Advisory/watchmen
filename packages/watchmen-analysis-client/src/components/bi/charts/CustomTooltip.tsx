...
export const CustomTooltip = React.memo(({ active, payload, label, format, unit }: TooltipProps & { format?: string; unit?: string }) => {
            ...
            <div key={index} className="flex items-center gap-2">
              ...
              <span className="font-medium text-popover-foreground tabular-nums">
                {typeof entry.value === 'number'
                  ? `${formatMetricValue(entry.value, format)}${unit ? ` ${unit}` : ''}`
                  : (entry.value ?? '-')}
              </span>
            </div>
          ))}
