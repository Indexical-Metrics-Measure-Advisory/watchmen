...
class TableExtractor:
    ...
    def count_data_volume(self, trigger_event: TriggerEvent, table_config: CollectorTableConfig) -> int:
        config = self.table_config_service.find_by_name(table_config.name, table_config.tenantId)
        base_criteria = self.get_criteria(trigger_event, config)
        data_count = ask_source_extractor(config).count_by_criteria(base_criteria)
        return data_count


def get_collector_table_extractor_service() -> TableExtractor:
