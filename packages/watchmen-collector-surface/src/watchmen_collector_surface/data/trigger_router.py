...
from watchmen_collector_kernel.storage import get_trigger_event_service, get_change_data_record_service, \
    get_change_data_json_service, get_scheduled_task_service, get_collector_table_config_service, \
    get_collector_model_config_service
...
@router.post('/collector/trigger/event/model', tags=[UserRole.ADMIN, UserRole.SUPER_ADMIN],
             response_model=TriggerEvent)
async def trigger_event_by_model(
        event: TriggerEventRequest, principal_service: PrincipalService = Depends(get_any_admin_principal)
) -> TriggerEvent:
    if event.startTime is None or event.endTime is None:
        raise_400('start time or end time  is required.')
    ...
    trigger_event.startTime = event.startTime
