...
def build_yes_no_item(token:str,lang:str = "en")->List[Any]:
    return [CopilotAnswerOption(text=get_message_by_lang(lang,"yes"), action="yes",token=token,vertical=True),CopilotAnswerOption(text=get_message_by_lang(lang,"no"), action="no",token=token)]