...
class HypothesisSimulationStep(SimulationStepInterface):
    ...
    def execute(self, challenge:BusinessChallengeWithHypotheses, context, *args, **kwargs):

        challenge.hypotheses = challenge.hypotheses or []
        number_of_hypothesis = len(challenge.hypotheses) if challenge.hypotheses else 0
        for _ in range(self.hypothesis_limit - number_of_hypothesis):
            ...
            current_hypothesis_markdown_table = generate_markdown_table_for_hypothesis(challenge.hypotheses)
