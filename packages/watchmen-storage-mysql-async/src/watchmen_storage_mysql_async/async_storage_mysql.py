...
from watchmen_storage_rds_async import AsyncTopicDataStorageRDS, AsyncStorageRDS
from watchmen_storage_mysql.table_creator import build_columns_script, build_indexes_script, \
	build_unique_indexes_script, build_table_script
from watchmen_storage_mysql.where_build import build_criteria_for_statement, build_literal

# noinspection DuplicatedCode
logger = getLogger(__name__)


class AsyncStorageMySQL(AsyncStorageRDS):
...
class AsyncTopicDataStorageMySQL(AsyncStorageMySQL, AsyncTopicDataStorageRDS):
	# noinspection SqlResolve
	async def create_topic_entity(self, topic: Topic) -> None:
		try:
			await self.connect()
			script = build_table_script(topic)
			await self.connection.execute(text(script))
	...
	async def update_topic_entity(self, topic: Topic, original_topic: Topic) -> None:
		...
		try:
			...
			await self.connection.execute(text(f"CALL DROP_INDEXES_ON_TOPIC_CHANGED('{entity_name}')"))
			# try to change column anyway, ignore when failed
			for column_script in build_columns_script(topic, original_topic):
				try:
					await self.connection.execute(text(column_script))
				except Exception as e:
					logger.error(e, exc_info=True, stack_info=True)
			# try to add index
			for unique_index_script in build_unique_indexes_script(topic):
				try:
					await self.connection.execute(text(unique_index_script))
				except Exception as e:
					logger.error(e, exc_info=True, stack_info=True)
			for index_script in build_indexes_script(topic):
				try:
					await self.connection.execute(text(index_script))
				except Exception as e:
					logger.error(e, exc_info=True, stack_info=True)
			try:
				# noinspection SqlResolve
				await self.connection.execute(text(f'ALTER TABLE {as_table_name(topic)} ADD INDEX (tenant_id_)'))
			except Exception as e:
				logger.error(e, exc_info=True, stack_info=True)
			try:
				# noinspection SqlResolve
				await self.connection.execute(text(f'ALTER TABLE {as_table_name(topic)} ADD INDEX (insert_time_)'))
			except Exception as e:
				logger.error(e, exc_info=True, stack_info=True)
			try:
				# noinspection SqlResolve
				await self.connection.execute(text(f'ALTER TABLE {as_table_name(topic)} ADD INDEX (update_time_)'))
			except Exception as e:
				logger.error(e, exc_info=True, stack_info=True)
		except Exception as e:
			logger.error(e, exc_info=True, stack_info=True)
