...
@router.get('/dqc/error/monitor', tags=[UserRole.ADMIN], response_model=None)
async def run_monitor(start_date: Optional[str], end_date: Optional[str],
                      principal_service: PrincipalService = Depends(get_admin_principal)) -> Optional[MonitorResult]:
	...
	pipeline_error_result: PipelineMonitorResult = await pipeline_health_service.run_health_check(start_query_date,
	                                                                                              end_query_date)
