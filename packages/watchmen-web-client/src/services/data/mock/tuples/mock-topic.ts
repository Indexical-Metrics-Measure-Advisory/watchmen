...
import { Topic, TopicId, TopicKind, TopicLayer, TopicType } from "../../tuples/topic-types";
...
export const fetchMockTopic = async (topicId: TopicId): Promise<{ topic: Topic }> => {
	...
	if (found) {
		const { topicId, name, kind, type, layer, description, factors, version, createdAt, lastModifiedAt } = found;
		topic = { topicId, name, kind, type, layer, description, factors, version, createdAt, lastModifiedAt };
	} else {
		topic = {
			topicId,
			name: "Mock Topic",
...
export const fetchMockTopicDataIds = async (topic: TopicId, condition?: ParameterJoint): Promise<Array<string>> => {
	return new Promise<Array<string>>((resolve) => {
		setTimeout(() => {
			resolve(
				new Array(Math.floor(Math.random() * 100) + 100).fill(1).map((_, index) => {
...
export const mockRerunTopic = async (topicId: TopicId, pipelineId: PipelineId, dataId: string): Promise<void> => {
	...
	if (value >= 0.05) {
		...
		return new Promise<void>((_, reject) =>
			setTimeout(() => reject(new Error("A mock error")), Math.floor(Math.random() * 500) + 500)
		);
	}
};
