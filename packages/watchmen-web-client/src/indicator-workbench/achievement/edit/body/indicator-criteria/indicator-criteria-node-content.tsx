...
const NameLabel = (props: {
	...
	if (topic == null && subject == null) {
		return <>{Lang.INDICATOR_WORKBENCH.ACHIEVEMENT.MISSED_INDICATOR_BASE}</>;
	} else if (criteria.length === 0) {
		return <>{Lang.INDICATOR_WORKBENCH.ACHIEVEMENT.NO_INDICATOR_CRITERIA_DEFINED}</>;
	} else if ((achievementIndicator.name || '').trim().length === 0) {
		return <span>{criteria.length} {Lang.INDICATOR_WORKBENCH.ACHIEVEMENT.INDICATOR_CRITERIA_DEFINED}</span>;
	} else {
