...
export const isConnectedSpaceOpened = (connectedSpaceId: ConnectedSpaceId, location: Location): boolean => {
	const match = matchPath<{ connectId: ConnectedSpaceId }>(location.pathname, Router.CONSOLE_CONNECTED_SPACE);
...
export const toSubjectReports = (connectedSpaceId: ConnectedSpaceId, subjectId: SubjectId) => {
	return Router.CONSOLE_CONNECTED_SPACE_SUBJECT_REPORTS
		.replace(':connectId', connectedSpaceId)
		.replace(':subjectId', subjectId);
};
export const toSubjectReport = (connectedSpaceId: ConnectedSpaceId, subjectId: SubjectId, reportId: ReportId) => {
	return Router.CONSOLE_CONNECTED_SPACE_SUBJECT_REPORT
		.replace(':connectId', connectedSpaceId)
		.replace(':subjectId', subjectId).replace(':reportId', reportId);
};
export const isDashboardOpened = (dashboardId: DashboardId, location: Location): boolean => {
	const match = matchPath<{ dashboardId: DashboardId }>(location.pathname, Router.CONSOLE_DASHBOARD);
	// eslint-disable-next-line
	return !!match && match.params.dashboardId == dashboardId;
};
export const toDashboard = (dashboardId: DashboardId) => {
	return Router.CONSOLE_DASHBOARD.replace(':dashboardId', dashboardId);
}
export const toPipeline = (pipelineId: PipelineId) => {
	return Router.ADMIN_PIPELINE.replace(':pipelineId', pipelineId);
}
export const toAchievementEdit = (achievementId: AchievementId) => {
	return Router.INDICATOR_ACHIEVEMENT_EDIT.replace(':achievementId', achievementId);
}