...
const ConvergenceCard = (props: { convergence: QueryConvergence }) => {
	const {convergence} = props;

	const {fire} = useTupleEventBus();

	const onEditClicked = () => {
		fire(TupleEventTypes.DO_EDIT_TUPLE, convergence);
	};

	return <TupleCard key={convergence.convergenceId} onClick={onEditClicked}>
		<TupleCardTitle>
			<span>{convergence.name}</span>
		</TupleCardTitle>
		<TupleCardDescription>{convergence.description}</TupleCardDescription>
		<TupleCardStatistics>
			<TupleCardStatisticsItem
				tooltip={{label: Lang.INDICATOR.CONVERGENCE.CREATE_AT, alignment: TooltipAlignment.CENTER}}>
				<FontAwesomeIcon icon={ICON_CREATED_AT}/>
				<span>{prettifyDateTimeToMinute(convergence.createdAt)}</span>
			</TupleCardStatisticsItem>
			<TupleCardStatisticsItem
				tooltip={{
				...
				<span>{prettifyDateTimeToMinute(convergence.lastModifiedAt)}</span>
			</TupleCardStatisticsItem>
		</TupleCardStatistics>
	</TupleCard>;
};

export const renderCard = (convergence: QueryConvergence) => {
	return <ConvergenceCard convergence={convergence}/>;
};