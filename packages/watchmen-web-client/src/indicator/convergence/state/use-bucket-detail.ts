...
export const useBucketDetail = () => {
	...
	useEffect(() => {
		...
		on(ConvergencesEventTypes.ASK_BUCKETS, onAskBucketDetails);
		on(ConvergencesEventTypes.ASK_BUCKET, onAskBucket);
		return () => {
			off(ConvergencesEventTypes.ASK_BUCKETS, onAskBucketDetails);
			off(ConvergencesEventTypes.ASK_BUCKET, onAskBucket);
		};
	}, [on, off, buckets]);
};