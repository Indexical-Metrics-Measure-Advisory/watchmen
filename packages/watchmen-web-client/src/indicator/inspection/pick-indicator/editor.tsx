...
export const PickIndicatorEditor = () => {
	...
	const onPickClicked = () => {
		if (selectedIndicatorId == null) {
			fireGlobal(EventTypes.SHOW_ALERT, <AlertLabel>
				{Lang.INDICATOR.INSPECTION.INDICATOR_IS_REQUIRED}
			</AlertLabel>);
			return;
		}
		...
		fire(InspectionEventTypes.ASK_INDICATOR, inspection!.indicatorId, (indicator: IndicatorForInspection) => {
			...
			} else if (picked.aggregateArithmetic == null) {
				inspection!.aggregateArithmetics = [picked.aggregateArithmetic];
			} else {
	...
	return <PickIndicatorContainer>
		<InspectionLabel>{Lang.INDICATOR.INSPECTION.PICK_INDICATOR_LABEL}</InspectionLabel>
		...
		<InspectionButton ink={ButtonInk.PRIMARY} onClick={onPickClicked}>
			{Lang.INDICATOR.INSPECTION.PICK_INDICATOR}
		</InspectionButton>
