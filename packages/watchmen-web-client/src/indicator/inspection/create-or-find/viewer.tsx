...
const NameDialog = (props: { inspection: Inspection; onNamed: () => void; }) => {
	...
	const onConfirmClicked = () => {
		if (name.trim().length === 0) {
			setError(Lang.INDICATOR.INSPECTION.NAME_IS_REQUIRED);
			return;
		}
	...
	return <>
		<DialogHeader>
			<DialogTitle>{Lang.INDICATOR.INSPECTION.SET_NAME_LABEL}</DialogTitle>
		</DialogHeader>
		<DialogBody>
...
export const CreateOrFindViewer = () => {
	...
	const name = inspection.name || Lang.INDICATOR.INSPECTION.NONAME_ON_INSPECTION;
	const hasName = !!inspection.name;
	const canSetName = !!inspection.indicatorId;

	return <CreateOrFindContainer>
		<InspectionLabel>{Lang.INDICATOR.INSPECTION.PICKED_INSPECTION_LABEL}</InspectionLabel>
		...
		{canSetName
			? <InspectionButton ink={ButtonInk.PRIMARY} onClick={onRenameClicked} data-hide-on-print={true}>
				{hasName ? Lang.INDICATOR.INSPECTION.RENAME : Lang.INDICATOR.INSPECTION.NEW_NAME}
			</InspectionButton>
			: null}
