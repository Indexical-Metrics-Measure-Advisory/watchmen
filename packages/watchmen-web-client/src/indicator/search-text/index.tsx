		...
		<SearchPopup>
			{result.searched
				? (result.items.length === 0
					? <CandidateItem active={false}>{Lang.INDICATOR.INDICATOR.NO_MATCHED}</CandidateItem>
				...
				: <OnSearching>
					{searchText.trim().length === 0
						? <>
							...
							<span>{Lang.INDICATOR.INDICATOR.WAIT_INPUT}</span>
						</>
						: <>
							<FontAwesomeIcon icon={ICON_LOADING} spin={true}/>
							<span>{Lang.INDICATOR.INDICATOR.SEARCHING}</span>
						</>}
				</OnSearching>}
		</SearchPopup>
	</SearchPart>;
