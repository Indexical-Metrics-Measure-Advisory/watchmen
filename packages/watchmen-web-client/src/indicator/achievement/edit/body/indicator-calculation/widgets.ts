...
import {AchievementBlock} from '../widgets';

export const IndicatorCalculationNodeContainer = styled.div.attrs({'data-widget': 'indicator-calculation-node-container'})`
	display  : block;
	position : relative;
`;
export const IndicatorCalculationNode = styled(AchievementBlock).attrs<{ expanded: boolean }>(
	({expanded}) => {
		return {
...
export const IndicatorCalculationValue = styled.span.attrs({'data-widget': 'indicator-calculation-value'})`
	...
	position    : relative;
	align-items : center;
	color       : var(--achievement-indicator-value-color);
...
export const IndicatorCalculationFormulaContainer = styled.div.attrs<{ expanded: boolean }>(
	...
	&:before {
		...
		width            : 100%;
		height           : 100%;
		background-color : var(--achievement-indicator-bg-color);
		z-index          : -1;
	}
	> textarea {
		font-size : 1em;
