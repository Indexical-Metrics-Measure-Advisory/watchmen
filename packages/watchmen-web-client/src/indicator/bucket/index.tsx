...
const validate = (bucket: Bucket): string | true => {
	if (!bucket.name || !bucket.name.trim()) {
		return Lang.INDICATOR.BUCKET.BUCKET_NAME_IS_REQUIRED;
	} else if (isMeasureBucket(bucket) && bucket.measure == null) {
		return Lang.INDICATOR.BUCKET.BUCKET_MEASURE_IS_REQUIRED;
	} else if (isEnumMeasureBucket(bucket) && (bucket.enumId || '').trim().length === 0) {
		return Lang.INDICATOR.BUCKET.BUCKET_ENUM_IS_REQUIRED;
	} else if (bucket.segments == null || bucket.segments.length === 0) {
		return Lang.INDICATOR.BUCKET.BUCKET_SEGMENTS_IS_REQUIRED;
	} else if (bucket.segments.some(segment => (segment.name ?? '').trim().length === 0)) {
		return Lang.INDICATOR.BUCKET.BUCKET_SEGMENT_NAME_IS_REQUIRED;
		...
		[...bucket.segments.map(({value}) => value)]
			...
			.some(({min, max}, index, values) => {
				if (index === 0) {
					...
					if (min != null) {
						message = Lang.INDICATOR.BUCKET.EMPTY_FIRST_MIN_OF_NUMERIC_SEGMENT;
						return true;
					}
					if (max == null) {
						message = Lang.INDICATOR.BUCKET.NOT_EMPTY_FIRST_MAX_OF_NUMERIC_SEGMENT;
						return true;
					} else if (!isXaNumber(max)) {
						message = Lang.INDICATOR.BUCKET.BE_NUMERIC_OF_NUMERIC_SEGMENT;
						return true;
					}
					return false;
				}
				...
				if (index !== lastIndex) {
					if (min == null || max == null) {
						message = Lang.INDICATOR.BUCKET.NOT_EMPTY_OF_NUMERIC_SEGMENT;
						return true;
					}
					...
					if ((min + '').localeCompare((max + ''), void 0, {numeric: true}) >= 0) {
						message = Lang.INDICATOR.BUCKET.MIN_MAX_ORDER_OF_NUMERIC_SEGMENT;
						return true;
					}
				} else {
					if (min == null) {
						message = Lang.INDICATOR.BUCKET.NOT_EMPTY_LAST_MIN_OF_NUMERIC_SEGMENT;
						return true;
					}
					if (!isXaNumber(min)) {
						message = Lang.INDICATOR.BUCKET.BE_NUMERIC_OF_NUMERIC_SEGMENT;
					...
					if (max != null) {
						message = Lang.INDICATOR.BUCKET.EMPTY_LAST_MAX_OF_NUMERIC_SEGMENT;
						return true;
					}
				}
				if (Number(min) > Number(values[index - 1].max)) {
					message = Lang.INDICATOR.BUCKET.RANGE_LACK_EXISTS_OF_NUMERIC_SEGMENT;
					return true;
				} else if (Number(min) < Number(values[index - 1].max)) {
					message = Lang.INDICATOR.BUCKET.RANGE_OVERLAP_EXISTS_OF_NUMERIC_SEGMENT;
					return true;
				}
				return false;
			});
		...
		if (Object.keys(categories).some(key => categories[key] > 1)) {
			return Lang.INDICATOR.BUCKET.NO_SHARED_OF_CATEGORY_SEGMENT;
		}
	}
...
const IndicatorBuckets = () => {
	...
	return <TupleWorkbench title={Lang.INDICATOR.BUCKET.TITLE}
	                       ...
	                       tupleLabel={Lang.INDICATOR.BUCKET.LABEL}
	                       newTupleLabelPrefix={Lang.INDICATOR.BUCKET.NEW_BUCKET_PREFIX}
	                       existingTupleLabelPrefix={Lang.INDICATOR.BUCKET.EXISTING_BUCKET_PREFIX}
...
const IndicatorBucketsIndex = () => {
	return <TupleEventBusProvider>
		<IndicatorBuckets/>
	</TupleEventBusProvider>;
};

export default IndicatorBucketsIndex;