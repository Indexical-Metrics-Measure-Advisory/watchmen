...
export const AvailableEnumItems = (props: { bucket: EnumMeasureBucket; enum?: EnumItems; }) => {
	...
	const onConfirmAddClicked = () => {
		if (selectedSegment == null) {
			fireGlobal(EventTypes.SHOW_ALERT, <AlertLabel>
				{Lang.INDICATOR.BUCKET.PLEASE_SELECT_SEGMENT}
			</AlertLabel>);
			return;
		} else if (selection.length === 0) {
			fireGlobal(EventTypes.SHOW_ALERT, <AlertLabel>
				{Lang.INDICATOR.BUCKET.PLEASE_SELECT_ENUM_ITEM}
			</AlertLabel>);
			return;
		}
		...
		.map((segment, segmentIndex) => {
			return {
				...
				label: () => {
					return {
						node: <>
							{segment.name || <>{Lang.INDICATOR.BUCKET.SEGMENT_LABEL} # {segmentIndex + 1}</>}
						</>,
	...
	return <AvailableEnumItemsContainer>
		<TuplePropertyLabel>{Lang.INDICATOR.BUCKET.AVAILABLE_ENUM_ITEMS_LABEL}</TuplePropertyLabel>
		<AvailableEnumItemSelectors>
			{availableItems.length === 0
				? <NoAvailableEnumItem>{Lang.INDICATOR.BUCKET.NO_AVAILABLE_ENUM_ITEMS}</NoAvailableEnumItem>
		...
		<AvailableItemOperator>
			<TuplePropertyDropdown value={selectedSegment} options={segmentOptions}
			                       please={Lang.INDICATOR.BUCKET.PLEASE_SELECT_SEGMENT}
			                       onChange={onSegmentChanged}/>
			<Button ink={ButtonInk.PRIMARY} onClick={onConfirmAddClicked}>
				{Lang.INDICATOR.BUCKET.ADD_AVAILABLE_ITEMS_INTO_SEGMENT}
			</Button>
		</AvailableItemOperator>
	</AvailableEnumItemsContainer>;
};