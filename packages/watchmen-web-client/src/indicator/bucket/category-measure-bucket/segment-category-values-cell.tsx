...
export const SegmentCategoryValuesCell = (props: { holder: CategorySegmentsHolder, segment: CategorySegment }) => {
	...
	const tryToAddValue = () => {
		...
		if (segment.value.includes(categoryValue)) {
			fireGlobal(EventTypes.SHOW_ALERT, <AlertLabel>
				{Lang.INDICATOR.BUCKET.DUPLICATE_CATEGORY_SEGMENT_VALUE}
			</AlertLabel>);
			return;
		} else if (segment.value.some(v => v.toUpperCase() === categoryValue.toUpperCase())) {
			fireGlobal(EventTypes.SHOW_YES_NO_DIALOG,
				Lang.INDICATOR.BUCKET.DUPLICATE_CATEGORY_SEGMENT_VALUE_CASE_IGNORED,
				() => {
					add(categoryValue);
			...
			fireGlobal(EventTypes.SHOW_ALERT, <AlertLabel>
				{Lang.INDICATOR.BUCKET.OTHERS_IS_EXCLUSIVE_ON_CATEGORY_SEGMENT}
			</AlertLabel>);
			return;
		} else {
			add(categoryValue);
	...
	return <SegmentValueCellContainer canAdd={!hasOthers}>
		<SegmentValues>
			{values.length === 0
				?
				<NoSegmentValueDefined>{Lang.INDICATOR.BUCKET.NO_SEGMENT_VALUE_DEFINED}</NoSegmentValueDefined>
				: values.map(value => {
