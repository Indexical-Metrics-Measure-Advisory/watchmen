...
export const SegmentsButton = (props: { bucket: Bucket }) => {
	...
	const buttonLabel = segmentCount === 0
		? Lang.INDICATOR.BUCKET.NO_SEGMENT_DEFINED
		: (segmentCount === 1
			? Lang.INDICATOR.BUCKET.ONE_SEGMENT_DEFINED
			: <>{segmentCount} {Lang.INDICATOR.BUCKET.N_SEGMENT_DEFINED}</>);

	return <>
		<TuplePropertyLabel>{Lang.INDICATOR.BUCKET.SEGMENTS_LABEL}</TuplePropertyLabel>
