...
const BucketCard = (props: { bucket: QueryBucket }) => {
	...
	return <TupleCard key={bucket.bucketId} onClick={onEditClicked}>
		...
		<TupleCardStatistics>
			<TupleCardStatisticsItem
				tooltip={{label: Lang.INDICATOR.BUCKET.CREATE_AT, alignment: TooltipAlignment.CENTER}}>
				<FontAwesomeIcon icon={ICON_CREATED_AT}/>
				<span>{prettifyDateTimeToMinute(bucket.createdAt)}</span>
			</TupleCardStatisticsItem>
			<TupleCardStatisticsItem
				tooltip={{label: Lang.INDICATOR.BUCKET.LAST_MODIFIED_AT, alignment: TooltipAlignment.CENTER}}>
