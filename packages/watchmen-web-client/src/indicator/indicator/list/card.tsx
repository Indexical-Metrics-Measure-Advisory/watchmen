...
const IndicatorCard = (props: { indicator: QueryIndicator }) => {
	const {indicator} = props;

	const {fire} = useTupleEventBus();

	const onEditClicked = () => {
		fire(TupleEventTypes.DO_EDIT_TUPLE, indicator);
	};

	return <TupleCard key={indicator.indicatorId} onClick={onEditClicked}>
		<TupleCardTitle>
			<span>{indicator.name}</span>
		</TupleCardTitle>
		<TupleCardDescription>{indicator.description}</TupleCardDescription>
		<TupleCardStatistics>
			<TupleCardStatisticsItem
				tooltip={{label: Lang.INDICATOR.INDICATOR.CREATE_AT, alignment: TooltipAlignment.CENTER}}>
				<FontAwesomeIcon icon={ICON_CREATED_AT}/>
				<span>{prettifyDateTimeToMinute(indicator.createdAt)}</span>
			</TupleCardStatisticsItem>
			<TupleCardStatisticsItem
				tooltip={{
					label: Lang.INDICATOR.INDICATOR.LAST_MODIFIED_AT,
...
export const renderCard = (indicator: QueryIndicator) => {
