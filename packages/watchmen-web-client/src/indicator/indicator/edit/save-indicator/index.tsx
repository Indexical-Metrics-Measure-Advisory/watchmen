	...
	const onSaveClicked = () => {
		...
		if (data!.indicator!.name == null || data!.indicator!.name.trim().length === 0) {
			fireGlobal(EventTypes.SHOW_ALERT,
				<AlertLabel>{Lang.INDICATOR.INDICATOR.REQUIRE_INDICATOR_NAME}</AlertLabel>);
			return;
		}
		...
		<StepTitle visible={visible && onEdit}>
			...
			{isOnCreate
				...
				: <SaveButton ink={ButtonInk.PRIMARY} onClick={onDiscardClicked}>
					{Lang.INDICATOR.INDICATOR.NOT_NOW}
				</SaveButton>}
			<SaveButton ink={ButtonInk.PRIMARY} onClick={onSaveClicked}>
				{saving ? <FontAwesomeIcon icon={ICON_LOADING} spin={true}/> : null}
				<span>{isOnCreate ? Lang.INDICATOR.INDICATOR.SAVE_INDICATOR : Lang.INDICATOR.INDICATOR.SAVE_NAME}</span>
			</SaveButton>
		</StepTitle>
		<StepTitle visible={visible && !onEdit}>
			<StepTitleButton ink={ButtonInk.PRIMARY} onClick={onChangeNameClicked}>
				{Lang.INDICATOR.INDICATOR.CHANGE_NAME}{name.length !== 0 ? ` [ ${name} ]` : ''}
			</StepTitleButton>
		</StepTitle>
	</Step>;
};