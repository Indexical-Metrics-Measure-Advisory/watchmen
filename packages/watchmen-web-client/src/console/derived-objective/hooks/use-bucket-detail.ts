...
export const useBucketDetail = () => {
	...
	useEffect(() => {
		...
		on(ObjectiveEventTypes.ASK_BUCKETS, onAskBucketDetails);
		on(ObjectiveEventTypes.ASK_BUCKET, onAskBucket);
		return () => {
			off(ObjectiveEventTypes.ASK_BUCKETS, onAskBucketDetails);
			off(ObjectiveEventTypes.ASK_BUCKET, onAskBucket);
		};
	}, [on, off, buckets]);
};