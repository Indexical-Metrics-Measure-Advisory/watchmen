...
DELIMITER |
CREATE FUNCTION WATCHMEN_MIGRATION_REDRESS_CONSTANT_FUNCTIONS(a_json CLOB)
    RETURNS CLOB
    NO SQL
BEGIN
    DECLARE func_body VARCHAR2(200) DEFAULT '\\([[:blank:]]*([^\)[[:blank:]]]+)[[:blank:]]*,[[:blank:]]*([^\)[[:blank:]]]+)[[:blank:]]*\\)';
    DECLARE first_not_date VARCHAR2(200) DEFAULT '\\(([^\)[[:digit:]]])([^\)]+),([^\)]+)\\)';
    DECLARE second_not_date VARCHAR2(200) DEFAULT '\\(([^\)]+),([^\)[[:digit:]]])([^\)]+)\\)';
