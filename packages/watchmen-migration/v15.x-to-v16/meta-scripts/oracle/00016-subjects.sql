...
ALTER TABLE subjects MODIFY name VARCHAR2(50);
...
ALTER TABLE subjects ADD (connect_id VARCHAR2(50) NULL);
ALTER TABLE subjects ADD auto_refresh_interval NUMBER(20) NULL;
ALTER TABLE subjects ADD (user_id VARCHAR2(50) NULL);
ALTER TABLE subjects RENAME COLUMN tenantid TO tenant_id;
ALTER TABLE subjects MODIFY (tenant_id VARCHAR2(50));
...
CREATE OR REPLACE PROCEDURE WATCHMEN_MIGRATION_COPY_SUBJECT_IDS IS
    CURSOR cur_list IS SELECT subject_id, reportids FROM subjects WHERE reportids IS NOT NULL;
    report_ids JSON_ARRAY_T;
    report_id VARCHAR2(50);
BEGIN
    FOR a_subject in cur_list LOOP
        report_ids := JSON_ARRAY_T(a_subject.reportids);
        FOR item_index IN 0 .. report_ids.GET_SIZE() - 1 LOOP
            report_id := report_ids.GET_STRING(item_index);
            report_id := TRIM(BOTH '"' FROM report_id);
            EXECUTE IMMEDIATE 'UPDATE reports SET subject_id = ''' || a_subject.subject_id || ''' WHERE report_id = ''' || report_id || '''';
            COMMIT;
        END LOOP;
    END LOOP;
END;
