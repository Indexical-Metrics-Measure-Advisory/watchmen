...
dependency_version_pattern = re.compile('^watchmen-[a-zA-Z]+(-[a-zA-Z]+)? = "[0-9]{1,2}\.[0-9]{1,2}\.[0-9]{1,2}"$')
dependency_version_option_pattern = re.compile('^watchmen-[a-zA-Z]+(-[a-zA-Z]+)? = { version = "[0-9]{1,2}\.[0-9]{1,2}\.[0-9]{1,2}", optional = true }$')
...
def match_pattern(text: str, version) -> str:
	...
	elif re.match(dependency_version_pattern, text, flags=0):
		return change_dependency_version(text, version, False)
	elif re.match(dependency_version_option_pattern, text, flags=0):
		return change_dependency_version(text, version, True)
	else:
		return text
